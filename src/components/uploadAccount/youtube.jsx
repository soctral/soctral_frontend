...
export const YoutubeMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  // 🔥 FIX: Use stable form state hook with correct YouTube metrics per spec
  const defaultState = {
    videoCount: '',
    engagements: '',
    channelViewCount: ''
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const metrics = [
        { key: "video_count", value: parseInt(formData.videoCount) || 0, type: "number" },
        { key: "engagements", value: parseInt(formData.engagements) || 0, type: "number" },
        { key: "channel_view_count", value: parseInt(formData.channelViewCount) || 0, type: "number" }
      ];
      ...
      console.error('❌ Error submitting YouTube metrics:', err);
    } finally {
      setLoading(false);
    }
  }, [formData, onSubmit]);
  ...
  const isFormValid = () => {
    return formData.videoCount.trim() !== '' &&
      ...
      formData.channelViewCount.trim() !== '';
  };
      ...
      <div className="bg-[#0D0D0D] rounded-lg p-6 w-full max-w-xl shadow-2xl my-8 max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Average Watch Time <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
            ...
            <input
              ...
              step="0.01"
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Average Watch Time"
            />
          </div>
...
export const YoutubeFilterForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  // 🔥 FIX: Use stable form state hook with correct YouTube filter fields
  const defaultState = {
    channelUsername: '',
    age: '',
    subscribers: '',
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      const filledFields = Object.values(formData).filter(value => value.toString().trim() !== '').length;
      const score = Math.round((filledFields / 5) * 100);

      const filters = [
        { key: "channel_username", value: formData.channelUsername, type: "string" },
        { key: "account_age_years", value: parseFloat(formData.age) || 0, type: "number" },
        { key: "subscribers", value: parseInt(formData.subscribers) || 0, type: "number" },
      ...
      console.error('❌ Error submitting YouTube filters:', err);
    } finally {
      setLoading(false);
    }
  }, [formData, onSubmit]);
  ...
  const isFormValid = () => {
    return formData.username.trim() !== '' &&
      formData.subscribersCount.trim() !== '' &&
      ...
      <div className="bg-[#0D0D0D] rounded-lg p-6 w-full max-w-lg shadow-2xl my-8 max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Subscribers Count <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
