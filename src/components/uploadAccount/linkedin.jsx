...
export const LinkedinMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  // 🔥 FIX: Use stable form state hook with correct LinkedIn fields per spec
  const defaultState = {
    connectionCount: '',
    experience: ''
  };
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const score = Math.round((filledFields / 2) * 100);

      const metrics = [
        { key: "connection_count", value: parseInt(formData.connectionCount) || 0, type: "number" },
        { key: "experience", value: formData.experience, type: "string" }
      ];
      ...
      console.error('❌ Error submitting LinkedIn metrics:', err);
    } finally {
      setLoading(false);
    }
  }, [formData, onSubmit]);
  ...
  const isFormValid = () => {
    return formData.connectionCount.trim() !== '' &&
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Connections <span className="text-primary"> *</span></h2>
            </div>
...
export const LinkedinFilterForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  // 🔥 FIX: Use stable form state hook with correct LinkedIn filter fields
  const defaultState = {
    username: '',
    followersCount: '',
    originalEmail: '',
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      const filledFields = Object.values(formData).filter(value => value.toString().trim() !== '').length;
      const score = Math.round((filledFields / 4) * 100);
      ...
      console.error('❌ Error submitting LinkedIn filters:', err);
    } finally {
      setLoading(false);
    }
  }, [formData, onSubmit]);
