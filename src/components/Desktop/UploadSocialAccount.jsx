/**
 * Upload Social Account Component (Refactored)
 * 
 * Uses unified form components instead of 40+ individual platform imports.
 * Supports all platforms from configuration.
 */
...
const UploadSocialAccount = ({ isOpen, onClose, walletData = null }) => {
  ...
  const handlePriceSubmit = async (e) => {
    ...
    try {
      ...
      const response = await marketplaceService.createSellOrder(sellOrderPayload);

      console.log('✅ Sell order created successfully:', response);
      ...
      setResultMessage('Your account has been listed successfully! You can now add another account.');
      ...
      // Show error modal
      setResultType('error');
      setResultMessage(error.message || 'Failed to create sell order. Please try again.');
      setShowResultModal(true);
    } finally {
      setSubmitting(false);
    }
                ...
                <>
                  {platformAccounts.length > 0 && (
                    <div className="mb-4 p-3 bg-[rgba(255,255,255,0.05)] rounded-lg">
                      <h4 className="text-xs font-semibold mb-2">Saved {selectedButton.charAt(0).toUpperCase() + selectedButton.slice(1)} Accounts ({platformAccounts.length})</h4>
                  ...
                  {currentAccountId && (
                    ...
                    >
                      + Add Another {selectedButton.charAt(0).toUpperCase() + selectedButton.slice(1)} Account
                    </button>
                  )}

                  <div className="flex items-center justify-between mb-4 p-3 rounded-lg">
                    <div className="relative w-20 h-20 lg:w-24 lg:h-24">
                      <div className={`w-full h-full border-4 ${getBorderColor(credibilityScore)} rounded-full shadow-inner shadow-lg flex justify-center items-center bg-transparent`}>
                    ...
                    <div className="flex-1 ml-3">
                      <h3 className="text-md font-semibold mb-4">Account Credibility</h3>
                      <p className="text-[#868686] text-xs"> Add more metrics and filters to enhance authenticity and attract better trades.</p>
                    </div>
                  </div>
                </>
              )}
              ...
              {selectedButton && (
                <>
                  <h3 className="text-sm font-semibold mt-5">
                    Upload {selectedButton.charAt(0).toUpperCase() + selectedButton.slice(1)} Account Metrics & Filters
                  </h3>
                  <div className="mt-4 p-3 bg-[rgba(255,255,255,0.05)] rounded-lg">
                    ...
                    >
                      <div className="text-left w-[252px]">
                        ...
                        <p className="text-xs font-normal text-[rgba(255,255,255,1)]">
                          Add at least three key metrics to upload your account
                    ...
                    >
                      <div className="text-left w-[252px]">
                        ...
                        <p className="text-xs font-normal text-[rgba(255,255,255,1)]">
                          Select at least three filters to help buyers find your account easily.
                        </p>
                      </div>
...
export default UploadSocialAccount;