...
export const QouraMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const metrics = [
        { key: "min_answers", value: parseInt(formData.answers), type: "number" },
        { key: "min_total_views", value: parseInt(formData.totalViews), type: "number" },
        { key: "requires_top_writer", value: formData.topWriter === 'yes', type: "boolean" },
        { key: "min_upvote_ratio", value: parseFloat(formData.upvoteRatio), type: "number" }
      ];
      ...
      console.error('❌ Error submitting Quora requirements:', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Answers <span className="text-primary"> *</span></h2>
            </div>
            ...
            <input
              ...
              placeholder="Enter Minimum Number of Answers"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Total Views <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Minimum Total Content Views
            </label>
            <input
              type="number"
              name="totalViews"
              ...
              onChange={handleInputChange}
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Total Views"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Requires Top Writer <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Require Top Writer Status?
            </label>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Upvote Ratio <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
            ...
            <input
              ...
              step="0.1"
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Upvote Ratio"
            />
          </div>
...
export const QouraFilterForm = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    username: '',
    followersCount: '',
    originalEmail: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('❌ Error submitting Quora filters:', err);
    } finally {
      setLoading(false);
    }
  };
