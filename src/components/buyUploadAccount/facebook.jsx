...
export const FacebookMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const metrics = [
        { key: "min_recent_posts", value: parseInt(formData.recentPosts), type: "number" },
        { key: "preferred_follower_demographics", value: formData.followerDemographics, type: "string" },
        { key: "requires_monetised", value: formData.monetised === 'yes', type: "boolean" },
        { key: "min_ad_performance", value: parseFloat(formData.adPerformance), type: "number" }
      ];
      ...
      console.error('❌ Error submitting Facebook requirements:', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Recent Posts <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Minimum Number of Recent Posts
            </label>
            <input
              type="number"
              name="recentPosts"
              ...
              placeholder="Enter Minimum Number of Recent Posts"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Ad Performance <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Minimum Ad Performance (%)
            </label>
            <input
              type="number"
              name="adPerformance"
              ...
              step="0.1"
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Ad Performance"
            />
          </div>
...
export const FacebookFilterForm = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    username: '',
    followersCount: '',
    originalEmail: '',
