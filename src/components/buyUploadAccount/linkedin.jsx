...
export const LinkedinMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const metrics = [
        { key: "min_connections", value: parseInt(formData.connections), type: "number" },
      ...
      console.error('❌ Error submitting LinkedIn requirements:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const isFormValid = () => {
    return formData.connections.trim() !== '' && 
           formData.posts.trim() !== '' && 
           formData.premium !== '' && 
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Connections <span className="text-primary"> *</span></h2>
            </div>
            ...
            <input
              ...
              placeholder="Enter Minimum Number of Connections"
            />
          </div>
...
export const LinkedinFilterForm = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    username: '',
    followersCount: '',
    originalEmail: '',
