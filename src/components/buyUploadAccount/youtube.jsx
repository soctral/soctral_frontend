...
export const YoutubeMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const metrics = [
        { key: "min_videos", value: parseInt(formData.videos), type: "number" },
        { key: "min_total_views", value: parseInt(formData.totalViews), type: "number" },
        { key: "requires_monetised", value: formData.monetised === 'yes', type: "boolean" },
        { key: "min_average_watch_time", value: parseFloat(formData.averageWatchTime), type: "number" }
      ];
      ...
      console.error('❌ Error submitting YouTube requirements:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const isFormValid = () => {
    return formData.videos.trim() !== '' && 
           ...
           formData.monetised !== '' && 
           formData.averageWatchTime.trim() !== '';
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Videos <span className="text-primary"> *</span></h2>
            </div>
            ...
            <input
              ...
              placeholder="Enter Minimum Number of Videos"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Average Watch Time <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Minimum Average Watch Time
            </label>
            <input
              type="number"
              ...
              step="0.1"
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Average Watch Time"
            />
          </div>
...
export const YoutubeFilterForm = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    username: '',
    followersCount: '',
    originalEmail: '',
