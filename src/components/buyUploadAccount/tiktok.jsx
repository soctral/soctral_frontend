...
export const TiktokMetricForm = ({ isOpen, onClose, onSubmit, existingData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const metrics = [
        ...
        { key: "min_total_likes", value: parseInt(formData.totalLikes), type: "number" },
        { key: "requires_monetised", value: formData.monetised === 'yes', type: "boolean" },
        { key: "min_average_engagement", value: parseFloat(formData.averageEngagement), type: "number" }
      ];
      ...
      console.error('❌ Error submitting TikTok requirements:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const isFormValid = () => {
    return formData.videos.trim() !== '' && 
           formData.totalLikes.trim() !== '' && 
           formData.monetised !== '' && 
           formData.averageEngagement.trim() !== '';
  };
      ...
      <div className="bg-[#0D0D0D] rounded-lg p-6 w-full max-w-xl shadow-2xl my-8 max-h-[calc(100vh-4rem)] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Total Likes <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
              Minimum Total Likes
            </label>
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Total Likes"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 text-sm mb-2">
              <img src={star} alt="" />
              <h2>Minimum Average Engagement <span className="text-primary"> *</span></h2>
            </div>
            <label className="block text-sm font-normal text-[#868686] mb-2">
            ...
            <input
              ...
              step="0.1"
              className="w-full px-3 py-4 bg-[rgba(255,255,255,0.1)] text-xs rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#613cd0]"
              placeholder="Enter Minimum Average Engagement"
            />
          </div>
...
export const TiktokFilterForm = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    username: '',
    followersCount: '',
    originalEmail: '',
