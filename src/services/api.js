...
class ApiService {
  constructor() {
    this.baseURL = 'https://soctra-api-6bcecb2e8189.herokuapp.com';
    this.defaultHeaders = {
  ...
  encryptRequestData(data, method, endpoint) {
    ...
    if (this.shouldSkipEncryption(endpoint) || !['POST', 'PUT', 'PATCH'].includes(method.toUpperCase())) {
    ...
    try {
      ...
      console.warn('⚠️ Request encryption failed, sending unencrypted:', error.message);
      return typeof data === 'string' ? data : null;
    }
  }
  ...
  decryptResponseData(data, endpoint) {
    ...
    const isAuthEndpoint = authEndpoints.some(path => endpoint.startsWith(path));

    // If it's an auth endpoint and data looks encrypted, force decrypt
    if (isAuthEndpoint && data && typeof data === 'object' && data.data && typeof data.data === 'string') {
      console.warn(`⚠️ ${endpoint} returned encrypted data - forcing decryption`);
      try {
    ...
    if (data && typeof data === 'object' && data.data && typeof data.data === 'string') {
      try {
        ...
        console.warn('⚠️ Response decryption failed, returning original:', error.message);
        return data;
      }
    }
  ...
  async request(endpoint, options = {}) {
    try {
      ...
      if (config.body && ['POST', 'PUT', 'PATCH'].includes(method.toUpperCase()) && !isFormData) {
        ...
        const encryptedBody = this.encryptRequestData(bodyData, method, endpoint);

        if (encryptedBody !== null) {
          config.body = encryptedBody;
        } else {
          config.body = typeof bodyData === 'string' ? bodyData : JSON.stringify(bodyData);
        }
      }
      ...
      try {
        ...
        console.warn('⚠️ Decryption failed, using raw data:', decryptError.message);
      }
      ...
      if (!response.ok) {
        ...
        if (typeof data === 'object' && data !== null) {
          errorMessage = data.message || data.error || data.msg || data.details || 'Unknown error';
          errorDetails = {
